...
var table = '<table id="table_info" class="bordered"><thead><tr><th>序号</th><th>材料名称</th><th>余量</th><th>创建时间</th><th>更新时间</th><th>历史记录</th></tr></thead></table>';
var info_tr = "<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td onclick='get_history({5})'>查看</td></tr>";
...
function get_history(repair_material_id){
	my_box('http://127.0.0.1:5000/materialHistory?repair_material_id=' + repair_material_id);
}
...
function add_info_tr(item, start){
	$("#table_info").append(info_tr.format(start + '-' + item.id, item.repair_material_name, item.repair_material_has_amount, item.create_time, item.update_time, item.id));
}
